...
## 七、分阶段实施计划（后端优先，含面试加分业务）

> 各阶段中的**业务亮点与面试怎么讲**见 → **[INTERVIEW_BUSINESS_HIGHLIGHTS.md](./INTERVIEW_BUSINESS_HIGHLIGHTS.md)**。
...
| **2. 表与实体（权限+文件）** | sys_user、sys_role、sys_menu、sys_user_role、sys_role_menu、sys_file 表与实体、Mapper | 权限与文件表齐全，可做 CRUD |
| **3. 认证与权限** | 登录、JWT、角色菜单关联、菜单树接口、当前用户信息/菜单；Spring Security 配置 | 登录与接口权限可用（✅ 已完成） |
| **4. 引入 Redis（学习重点）** | Docker 加 Redis；登录限流、JWT 黑名单（登出）、用户权限/菜单缓存、接口限流 | Redis 贯穿认证与权限，形成模板 |
| **5. 审批流（面试加分）** | biz_approval_definition / instance / task 表与实体；流程定义、实例、待办/已办接口；**状态机**控制流转；与业务单（如订单）关联 | 可配置审批流，支持多节点与驳回 |
| **6. 订单与商品（面试加分）** | biz_product、biz_order、biz_order_item 表；**订单状态机**（待支付→已支付→待发货→…）；**库存乐观锁**；**超时关单**并回滚库存；**下单幂等**（clientToken）；订单可关联审批实例；商品/订单可走 Redis 缓存 | 完整订单+库存+状态机+幂等 |
| **7. 操作日志 / 审计（面试加分）** | sys_operation_log 表；AOP 记录关键操作（登录、审批、订单状态变更）；敏感信息不落库 | 谁在何时对谁做了什么，可审计 |
| **8. 上传下载 + MinIO** | Docker 加 MinIO；上传/下载接口、sys_file；可选：订单附件、审批附件 | 文件能力闭环 |
| **9. 数据权限（可选，面试加分）** | 角色/用户数据范围（本人/本部门/全部）；查询时按范围动态拼 WHERE 条件 | 不同角色看不同范围数据 |
| **10. 前端（后期）** | React 后台对接现有接口；登录、菜单树、权限、审批、订单、上传下载 | 全栈闭环 |

**当前进度**：阶段 1～3 已完成；下一步建议 **阶段 4（Redis）** 或 **阶段 5（审批流）**。

---

## 八、接下来可做的事

1. **阶段 4**：引入 Redis，实现登录限流、JWT 黑名单、菜单/权限缓存、接口限流。
2. **阶段 5**：审批流表与接口，状态机控制流转，为订单等业务提供「提交→审批」能力。
3. **阶段 6**：订单与商品、状态机、库存乐观锁、超时关单、下单幂等，并与审批流关联。
4. 后续按上表顺序推进阶段 7～10；**面试可讲的业务与亮点**见 [INTERVIEW_BUSINESS_HIGHLIGHTS.md](./INTERVIEW_BUSINESS_HIGHLIGHTS.md)。
